...
5. Inspect one PR and its checks with `repo_remote_status`, or audit bounded PR sets with `repo_remote_pull_requests`. Audit standalone branches with `repo_branch_audit`; it reports exact refs, base ancestry, ahead/behind counts, and open PR use without mutation. Draft-ready uses the fixed GitHub CLI wrapper. An explicitly owner-approved open, unmerged PR can be closed with `repo_write_retire_pull_request`, while a fully base-contained standalone branch can be removed with `repo_write_retire_branch` after exact branch/base SHA verification.
