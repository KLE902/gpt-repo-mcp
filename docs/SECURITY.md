...
`repo_write_pull_request_state` requires the exact PR head before marking a draft ready or closing an unmerged PR. `repo_write_finalize_pull_request` requires a confirmed merged PR, explicit owner approval, a clean worktree, exact local HEAD, and matching local/remote feature refs before synchronization, switch, or deletion. `repo_write_dispatch_workflow` dispatches only a named GitHub Actions workflow and bounded string inputs.
