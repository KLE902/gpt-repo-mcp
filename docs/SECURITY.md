...
No arbitrary shell, caller-supplied command runner, arbitrary Git tool, or force operation is registered. The optional script runner accepts only a configured `script_id`; executable, arguments, timeout, output limit, and inherited environment names come from local policy. The dedicated `repo_start_codex_task` tool can start only an existing verified `repo_write_codex_task` run. Its Codex executable, non-interactive arguments, `workspace-write` sandbox, repository root, prompt transport, timeout, output limit, and inherited environment names are server-owned. Existing-branch switch and branch deletion are fixed-purpose guarded operations. Standalone branch deletion requires a clean non-current branch, exact repository/branch/base SHAs, matching local and remote refs when both exist, proof that the branch commit is contained in the exact remote base, no open pull request, and explicit owner approval. Local Git tools use fixed argument arrays through `execFile`; GitHub PR/check/merge/workflow calls use explicit API operations. On Windows, server bootstrap first reuses a complete user-level Git identity. When none exists, it creates an isolated runtime Git home under local application data using optional `GPT_REPO_GIT_AUTHOR_NAME`/`GPT_REPO_GIT_AUTHOR_EMAIL` values or a deterministic `local.invalid` fallback. Repository and global Git configuration remain unchanged. Advisory tools such as `repo_change_plan` and `repo_next_action` return plans and recommendations only; they do not write files or run tests.

`repo_prepare_codex_task` renders a prompt in tool output. `repo_write_codex_task` creates a new local run with `PROMPT.md` and schema-v2 `run.json`, including repository/run identity, exact prompt/result paths, prompt SHA-256, allowed and forbidden paths, and verification commands. It refuses to overwrite any existing run artifact. `repo_start_codex_task` is a separate disabled-by-default opt-in for that exact existing run. It requires a clean non-base branch, exact branch and HEAD, verified manifest identity and prompt hash, non-empty path scope, gitignored execution artifacts, no prior result or execution state, and a single-writer repository lock. CLI flag detection is not sufficient: the start preflight must also complete read-only authentication plus one isolated real `workspace-write` operation, remove its probe artifact, and reverify unchanged branch, HEAD, index, and worktree before execution state is created. The MCP caller cannot supply prompt text, command, arguments, model, reasoning level, sandbox, timeout, environment, working directory, verification commands, or Git delivery instructions.

The start call creates `execution.json`, starts a detached bounded runner, and returns only after the runner confirms `running` or reaches a terminal state. The runner writes bounded redacted `stdout.jsonl` and `stderr.log`, classifies structured JSONL and known sandbox diagnostics, kills the whole process tree at timeout, and never retries automatically. Timeout, incomplete or malformed output, process/sandbox bootstrap failure, fallback-tool or unknown-write provenance, nonzero exit, invalid result, and Git/path contract violations all outrank normal success. A zero exit code, valid result, or correct diff cannot compensate for a broken execution boundary. Evidence is preserved and no automatic restore occurs. `repo_codex_review` exposes sandbox and boundary evidence directly with the normal Git review. Older durable state without boundary evidence is unverified, while legacy manually produced `RESULT.md` remains readable when no `execution.json` exists. A repository may also separately opt in to other fixed wrappers through `repo_run_allowed_script`. Arbitrary model-supplied commands or provider arguments remain unavailable.
...
All Git and process operations are disabled by default for every repo. New feature-branch creation, branch management, local stage/commit, push, PR create/update, PR merge, workflow dispatch, base synchronization, allowlisted script execution, durable Codex task execution, and cleanup each have explicit operation-policy toggles. `operations.enabled: true` is necessary but not sufficient. Durable Codex execution additionally requires `codex_task_run_enabled: true`; `codex_task_max_runtime_ms`, `codex_task_max_output_bytes`, and `codex_task_inherit_env` are local server-owned policy rather than caller inputs. Keep durable execution disabled in any environment with an unresolved sandbox bootstrap failure. Repair the local official Codex sandbox installation separately and require a new isolated smoke before enabling real work.
