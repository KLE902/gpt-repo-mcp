...
When work is still on a base branch, the supported sequence is `repo_write_create_branch` → `repo_git_review` → local commit → `repo_write_push` → `repo_write_pull_request` → `repo_remote_status` → explicit owner approval → `repo_write_merge_pull_request` → `repo_write_finalize_pull_request`. If work already starts on a suitable feature branch, skip only the create-branch step. Use `repo_write_sync_base` for local base synchronization without switching or deletion. When the checked-out feature branch must incorporate a newer base, use `repo_write_update_branch_from_base` with exact current feature/base SHAs; conflicts are reported without entering merge state.

Delivery operations are independent opt-ins: `git_branch_enabled`, `git_branch_manage_enabled`, `git_push_enabled`, `github_pull_request_enabled`, `github_workflow_dispatch_enabled`, `github_merge_enabled`, and `git_sync_enabled`. Remote operations accept only `origin` and GitHub.com repository URLs. Push requires a clean named feature branch, exact branch and HEAD guards, and a non-force refspec; `main` and `master` are rejected. Bounded PR listing uses `repo_remote_pull_requests` with fixed structured GitHub CLI arguments. Standalone branch cleanup begins with read-only `repo_branch_audit`; `repo_write_retire_branch` then requires exact repository, branch, and base SHAs, strict containment proof through ancestry, patch-equivalence, or an exact merged pull-request head, no open PR use, and explicit owner approval before deleting only verified refs. Draft-ready uses the fixed `github.pr-ready` wrapper. Closing an open unmerged PR requires explicit owner approval through `repo_write_retire_pull_request`; cleanup requires the exact PR head, no competing open PR on the same branch, and separate guarded local/origin deletion. Merge and post-merge deletion require explicit owner approval. Base synchronization is fast-forward only. Feature-branch synchronization uses exact feature/base SHA guards, merge-tree conflict preflight, and fixed fast-forward or merge arguments; no rebase, cherry-pick, force update, or push is available.
