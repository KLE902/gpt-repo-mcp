...
Read-only local Git status and diff operations are owned by `GitService`. Safe local mutations remain separate opt-in tools. `RemoteGitService` owns the bounded delivery layer: exact-guarded branch creation and switching, branch inspection, remote status, feature-branch push, PR create/update, owner-approved PR merge, verified post-merge finalization, workflow dispatch, and fast-forward base synchronization. `GitHubClient` owns the explicit GitHub REST operations plus runtime authentication. `AllowedScriptService` runs only locally configured script ids with fixed commands, arguments, environment inheritance, timeout, output, and HEAD guards. Advisory services call existing factual services where practical instead of bypassing repository policy.
