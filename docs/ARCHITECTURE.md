...
Read-only local Git status and diff operations are owned by `GitService`. Safe local mutations remain separate opt-in tools. `RemoteGitService` owns the bounded delivery layer: exact-guarded creation of a new local feature branch, remote status, feature-branch push, PR create/update, owner-approved PR merge, and fast-forward base synchronization. `GitHubClient` owns the explicit REST endpoints and runtime authentication. Advisory services call existing factual services where practical instead of bypassing repository policy.
...
When work starts on a base branch, the reviewed delivery continuation begins with `repo_write_create_branch`, which can only create and switch to a new branch from the exact current source branch and HEAD. It may carry the reviewed index/worktree state, but cannot open an existing branch or rewrite history. After local review and commit, delivery continues autonomously through `repo_write_push` → `repo_write_pull_request` → `repo_remote_status`; it then stops for explicit owner approval before `repo_write_merge_pull_request`. Remote state changes use exact local and pull-request head guards. The merge service treats GitHub merge and local base synchronization as separate outcomes so a post-merge local fetch problem cannot erase the fact that GitHub already merged the PR.
