...
import { BranchAuditInputSchema, BranchAuditResultSchema, RetireBranchInputSchema, RetireBranchResultSchema } from "../src/contracts/branch-lifecycle.contract.js";
...
describe("tool catalog contracts", () => {
  ...
  test("mutating tools use central contracts and annotations", () => {
    ...
    const createBranch = toolCatalog.find((tool) => tool.name === "repo_write_create_branch");
    expect(createBranch).toBeDefined();
    expect(createBranch?.inputSchema).toBe(CreateBranchInputSchema);
    expect(createBranch?.outputSchema).toBe(CreateBranchResultSchema);
    expect(createBranch?.annotations).toEqual(writeAnnotations);
  ...
  test("mutating tool schemas describe every input and output field", () => {
    ...
    expectFieldDescriptions([
      ...Object.entries(CreateBranchInputSchema.shape).map(([field, schema]) => [`repo_write_create_branch.${field}`, schema] as [string, { description?: string }]),
      ...Object.entries(CreateBranchResultSchema.shape).map(([field, schema]) => [`repo_write_create_branch.${field}`, schema] as [string, { description?: string }])
    ]);
  });
  ...
  test("delivery tool surface stays exact with local branch creation and open-world remote operations", () => {
    const remoteTools = toolCatalog
      .filter((tool) => ["repo_write_create_branch", "repo_remote_status", "repo_remote_pull_requests", "repo_write_push", "repo_write_pull_request", "repo_write_retire_pull_request", "repo_write_sync_base", "repo_write_update_branch_from_base", "repo_write_merge_pull_request", "repo_branch_audit", "repo_write_retire_branch"].includes(tool.name))
      .map((tool) => ({
        name: tool.name,
    ...
    expect(remoteTools).toEqual([
      {
        ...
        inputKeys: ["branch", "dry_run", "expected_head_sha", "expected_source_branch", "reason", "repo_id"],
        outputKeys: ["branch", "created", "dry_run", "head_sha", "ok", "source_branch", "warnings", "worktree_clean"]
      },
      {
        name: "repo_remote_status",
        annotations: remoteReadAnnotations,
        inputKeys: ["pull_number", "remote", "repo_id"],
        outputKeys: ["ahead", "behind", "branch", "checks", "clean", "head_sha", "ok", "pull_request", "pushed", "remote", "remote_head_sha", "repository", "upstream", "warnings"]
      },
      {
        name: "repo_remote_pull_requests",
        annotations: remoteReadAnnotations,
        inputKeys: ["base", "head", "include_checks", "limit", "remote", "repo_id", "state"],
        outputKeys: ["base", "head", "ok", "pull_requests", "remote", "repository", "state", "truncated", "warnings"]
      },
      {
        name: "repo_write_push",
        annotations: remoteWriteAnnotations,
        inputKeys: ["dry_run", "expected_branch", "expected_head_sha", "reason", "remote", "repo_id", "set_upstream"],
        outputKeys: ["branch", "dry_run", "head_sha", "ok", "pushed", "remote", "remote_head_sha", "repository", "upstream", "warnings"]
      },
      {
      ...
      },
      {
        ...
        annotations: remoteWriteAnnotations,
        inputKeys: ["comment", "delete_local_branch", "delete_remote_branch", "dry_run", "expected_head_sha", "expected_pull_head_sha", "owner_approved", "pull_number", "reason", "remote", "repo_id"],
        outputKeys: ["closed", "comment_added", "dry_run", "local_branch_deleted", "ok", "pull_request", "remote_branch_deleted", "warnings"]
      },
      {
        name: "repo_write_sync_base",
        annotations: remoteWriteAnnotations,
        inputKeys: ["base", "dry_run", "expected_head_sha", "reason", "remote", "repo_id"],
      ...
      },
      {
        ...
        outputKeys: ["checks", "dry_run", "merge_method", "merge_sha", "merged", "message", "ok", "pull_request", "sync", "warnings"]
      },
      {
  ...
  test("remote workflow schemas describe every top-level input and output field", () => {
    expectFieldDescriptions([
      ...Object.entries(CreateBranchInputSchema.shape).map(([field, schema]) => [`repo_write_create_branch.${field}`, schema] as [string, { description?: string }]),
      ...Object.entries(CreateBranchResultSchema.shape).map(([field, schema]) => [`repo_write_create_branch.${field}`, schema] as [string, { description?: string }]),
      ...Object.entries(RemoteStatusInputSchema.shape).map(([field, schema]) => [`repo_remote_status.${field}`, schema] as [string, { description?: string }]),
      ...Object.entries(RemoteStatusResultSchema.shape).map(([field, schema]) => [`repo_remote_status.${field}`, schema] as [string, { description?: string }]),
      ...Object.entries(PullRequestListInputSchema.shape).map(([field, schema]) => [`repo_remote_pull_requests.${field}`, schema] as [string, { description?: string }]),
      ...Object.entries(PullRequestListResultSchema.shape).map(([field, schema]) => [`repo_remote_pull_requests.${field}`, schema] as [string, { description?: string }]),
      ...Object.entries(PushInputSchema.shape).map(([field, schema]) => [`repo_write_push.${field}`, schema] as [string, { description?: string }]),
      ...Object.entries(PushResultSchema.shape).map(([field, schema]) => [`repo_write_push.${field}`, schema] as [string, { description?: string }]),
      ...Object.entries(PullRequestInputSchema.shape).map(([field, schema]) => [`repo_write_pull_request.${field}`, schema] as [string, { description?: string }]),
      ...Object.entries(PullRequestResultSchema.shape).map(([field, schema]) => [`repo_write_pull_request.${field}`, schema] as [string, { description?: string }]),
      ...Object.entries(RetirePullRequestInputSchema.shape).map(([field, schema]) => [`repo_write_retire_pull_request.${field}`, schema] as [string, { description?: string }]),
      ...
      ...Object.entries(BranchAuditInputSchema.shape).map(([field, schema]) => [`repo_branch_audit.${field}`, schema] as [string, { description?: string }]),
      ...Object.entries(BranchAuditResultSchema.shape).map(([field, schema]) => [`repo_branch_audit.${field}`, schema] as [string, { description?: string }]),
      ...Object.entries(RetireBranchInputSchema.shape).map(([field, schema]) => [`repo_write_retire_branch.${field}`, schema] as [string, { description?: string }]),
      ...Object.entries(RetireBranchResultSchema.shape).map(([field, schema]) => [`repo_write_retire_branch.${field}`, schema] as [string, { description?: string }])
    ]);
  });

  test("exposed tool surface shape stays stable", () => {
    expect(toolCatalog.filter((tool) => !tool.name.startsWith("repo_remote_") && !["repo_write_create_branch", "repo_git_branches", "repo_branch_audit", "repo_write_retire_branch", "repo_write_switch_branch", "repo_write_push", "repo_write_pull_request", "repo_write_retire_pull_request", "repo_write_finalize_pull_request", "repo_write_dispatch_workflow", "repo_run_allowed_script", "repo_write_sync_base", "repo_write_update_branch_from_base", "repo_write_merge_pull_request"].includes(tool.name)).map((tool) => ({
      name: tool.name,
      title: tool.title,
      ...
      [
        ...
        },
        {
          ...
          "description": "Use this when the owner explicitly starts the ratified ATO-001 PKR-004 Claude transport measurement. It accepts no parameters and owns the exact prompt, repository, Git, context, Claude, read-only invocation, runtime, output, artifact, and live-worktree lease boundaries.",
          "inputKeys": [],
        ...
        },
        {
          ...
          "description": "Use this when collecting or reviewing the fixed ATO-001 PKR-004 Claude transport result. It accepts no parameters, revalidates every repository and context boundary, returns invalid output only as diagnostic evidence, and releases the persistent read lease after terminal review.",
          "inputKeys": [],
